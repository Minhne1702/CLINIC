...
<div class="page-toolbar">
  ...
  <div class="page-toolbar__right"><a href="{$BASE_URL}/?role=receptionist&page=walk-in" class="btn-admin-primary"><i class="fa-solid fa-person-walking-arrow-right"></i> Đăng ký tại chỗ</a></div>
</div>
...
<div class="status-tabs mb-1">
  ...
  <a href="{$BASE_URL}/?role=receptionist&page=appointments" class="status-tab {if $cur==''}active{/if}">Tất cả <span class="tab-count">{$count.all|default:0}</span></a>
  <a href="{$BASE_URL}/?role=receptionist&page=appointments&status=pending" class="status-tab {if $cur=='pending'}active{/if}">Chờ xác nhận <span class="tab-count tab-count--warning">{$count.pending|default:0}</span></a>
  <a href="{$BASE_URL}/?role=receptionist&page=appointments&status=confirmed" class="status-tab {if $cur=='confirmed'}active{/if}">Đã xác nhận <span class="tab-count tab-count--blue">{$count.confirmed|default:0}</span></a>
</div>
<div class="admin-card mb-1"><div class="admin-card__body">
  <form method="GET" action="{$BASE_URL}/" class="filter-bar"><input type="hidden" name="role" value="receptionist"><input type="hidden" name="page" value="appointments">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><div class="table-actions">
        {if $apt.status=='pending'}<a href="{$BASE_URL}/?role=receptionist&page=appointments&action=confirm&id={$apt._id}" class="action-btn action-btn--success" title="Xác nhận"><i class="fa-solid fa-check"></i></a>{/if}
        <a href="{$BASE_URL}/?role=receptionist&page=checkin&q={$apt.patient_code}" class="action-btn" title="Check-in"><i class="fa-solid fa-right-to-bracket"></i></a>
        <a href="{$BASE_URL}/?role=receptionist&page=appointments&action=cancel&id={$apt._id}" class="action-btn action-btn--danger" title="Hủy" onclick="return confirm('Hủy lịch hẹn này?')"><i class="fa-solid fa-ban"></i></a>
      </div></td>
    </tr>
