...
<div class="status-tabs mb-1">
  ...
  <a href="/CLINIC/public/?role=pharmacist&page=prescriptions" class="status-tab {if $cur==''}active{/if}">Tất cả <span class="tab-count">{$count.all|default:0}</span></a>
  <a href="/CLINIC/public/?role=pharmacist&page=prescriptions&status=pending" class="status-tab {if $cur=='pending'}active{/if}">Chờ phát <span class="tab-count tab-count--warning">{$count.pending|default:0}</span></a>
  <a href="/CLINIC/public/?role=pharmacist&page=prescriptions&status=dispensing" class="status-tab {if $cur=='dispensing'}active{/if}">Đang bốc <span class="tab-count tab-count--blue">{$count.dispensing|default:0}</span></a>
  <a href="/CLINIC/public/?role=pharmacist&page=prescriptions&status=done" class="status-tab {if $cur=='done'}active{/if}">Đã phát <span class="tab-count tab-count--success">{$count.done|default:0}</span></a>
</div>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><div class="table-actions">
          <a href="/CLINIC/public/?role=pharmacist&page=dispensing&id={$rx._id}" class="btn-admin-primary" style="font-size:12px;padding:.35rem .75rem"><i class="fa-solid fa-capsules"></i> Phát thuốc</a>
          <a href="/CLINIC/public/?role=pharmacist&page=prescriptions&action=view&id={$rx._id}" class="action-btn" title="Xem"><i class="fa-solid fa-eye"></i></a>
        </div></td>
      </tr>
