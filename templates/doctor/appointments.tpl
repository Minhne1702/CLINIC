...
<div class="status-tabs mb-1">
  ...
  <a href="{$BASE_URL}/?role=doctor&page=appointments" class="status-tab {if $cur==''}active{/if}">
    Tất cả <span class="tab-count">{$count.all|default:0}</span>
  </a>
  <a href="{$BASE_URL}/?role=doctor&page=appointments&status=confirmed" class="status-tab {if $cur=='confirmed'}active{/if}">
    Đã xác nhận <span class="tab-count tab-count--blue">{$count.confirmed|default:0}</span>
  </a>
  <a href="{$BASE_URL}/?role=doctor&page=appointments&status=pending" class="status-tab {if $cur=='pending'}active{/if}">
    Chờ xác nhận <span class="tab-count tab-count--warning">{$count.pending|default:0}</span>
  </a>
  <a href="{$BASE_URL}/?role=doctor&page=appointments&status=completed" class="status-tab {if $cur=='completed'}active{/if}">
    ...
    <form method="GET" action="{$BASE_URL}/" class="filter-bar">
      ...
      <div class="filter-bar__group">
        <div class="filter-input">
          ...
          <input type="text" name="q" placeholder="Tên bệnh nhân..." value="{$filter.q|default:''}">
        </div>
    ...
    <table class="admin-table">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <div class="table-user">
              ...
              <div>
                ...
                <small>{$apt.patient_code}</small>
              </div>
            </div>
          </td>
          <td>
            <strong>{$apt.date|date_format:"%d/%m/%Y"}</strong><br>
          ...
          <td>
            <span style="font-size:13px">{$apt.symptoms|truncate:40:'...'|default:'—'}</span>
          </td>
          <td>
            ...
            {else}
              <span class="badge badge--neutral">{$apt.status}</span>
            {/if}
          </td>
          <td>
            <div class="table-actions">
              
              {* Cập nhật Logic: Khám Online vs Xem EMR Trực tiếp *}
              {if $apt.type == 'online' && $apt.status == 'confirmed'}
                <a href="{$BASE_URL}/?role=doctor&page=telemedicine&apt_id={$apt._id}" class="btn-admin-primary" style="font-size:12px;padding:.35rem .75rem; background: #8b5cf6; border-color: #8b5cf6;">
                  <i class="fa-solid fa-video"></i> Khám Online
                </a>
              {else}
                <a href="{$BASE_URL}/?role=doctor&page=records&patient_id={$apt.patient_id}" class="btn-admin-secondary" style="font-size:12px;padding:.35rem .75rem">
                  <i class="fa-solid fa-folder-open"></i> Hồ sơ
                </a>
              {/if}

              <a href="{$BASE_URL}/?role=doctor&page=appointments&action=view&id={$apt._id}" class="action-btn" title="Xem chi tiết">
